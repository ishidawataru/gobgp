...
func hasOwnASLoop(ownAS uint32, limit int, m *bgp.BGPUpdate) bool {
	cnt := 0
	...
	if err != nil {
		...
		if h.fsm.state == bgp.BGP_FSM_ESTABLISHED {
			...
			case bgp.BGP_MSG_UPDATE:
				...
				if !hasOwnASLoop(h.fsm.peerInfo.LocalAS, int(h.fsm.pConf.AsPathOptions.Config.AllowOwnAs), body) {
